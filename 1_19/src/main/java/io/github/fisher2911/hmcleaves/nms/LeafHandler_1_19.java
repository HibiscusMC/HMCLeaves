...
public class LeafHandler_1_19 implements LeafHandler {
    ...
    public LeafHandler_1_19(JavaPlugin plugin, LeafDataSupplier leafDataSupplier, PDCHelper pdcHelper) {
        this.plugin = plugin;
        ...
        Bukkit.getScheduler().runTaskLaterAsynchronously(this.plugin, () -> {
            ...
            Bukkit.getScheduler().runTaskLater(this.plugin, () -> {
                for (BlockPos pos : positions) {
                    ...
                    if (!(state.getBlock().getClass().equals(net.minecraft.world.level.block.LeavesBlock.class))) {
                        continue;
                    }

                    final BlockState newState = this.previousToNew.get(state.getBlock()).defaultBlockState();
                    final FakeLeafData data = this.leafDataSupplier.getDefault();
                    newState.setValue(LeavesBlock.PERSISTENT, data.fakePersistence());
                    newState.setValue(LeavesBlock.DISTANCE, data.fakeDistance());
                    newState.setValue(LeavesBlock.WATERLOGGED, state.getValue(LeavesBlock.WATERLOGGED));
                    ...
                    final PersistentDataContainer blockData = new CustomBlockData(
                            chunk.getBlock(pos.getX(), pos.getX(), pos.getZ()),
                            this.plugin
                    );
...
//                    blockData.set(PDCUtil.ACTUAL_DISTANCE_KEY, PersistentDataType.BYTE, (byte) data.actualDistance());
    ...
    private void loadPDCData(Chunk chunk) {
        ...
        for (var block : blocks) {
            ...
            this.leafDataSupplier.set(world, location.getBlockX(), location.getBlockY(), location.getBlockZ(), this.pdcHelper.getFakeLeafData(blockData));
        }
    }
